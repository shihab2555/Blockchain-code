...
func main() {
	t := time.Now()
	genesisBlock := Block{}
	genesisBlock = Block{0, t.String(), 0, calculateHash(genesisBlock), "", difficulty, "", 10000}
	manager = valid
	Blockchain = append(Blockchain, genesisBlock)
	log.Println("%d is MANAGER!", manager)
...
func calculateHash(block Block) string {
	record := strconv.Itoa(block.Index) + block.Timestamp + strconv.Itoa(block.TR) + block.PrevHash + block.Nonce
	h := sha256.New()
	h.Write([]byte(record))
...
func generateBlock(oldBlock Block, TR int) Block {
